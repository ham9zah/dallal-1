...
export default function CreateListing() {
  ...
  // جلب التصنيفات والمدن عند تحميل الصفحة
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!categoriesResponse.ok || !citiesResponse.ok) {
  ...
  // جلب الأحياء عند اختيار المدينة
  useEffect(() => {
    if (selectedCity) {
      const fetchDistricts = async () => {
        try {
          const response = await fetch(`http://localhost:8000/api/cities/${selectedCity}/districts`);
          if (!response.ok) {
            throw new Error('فشل في جلب الأحياء');
          }
          const data = await response.json();
          setDistricts(data.data);
        } catch (error) {
          console.error('خطأ في جلب الأحياء:', error);
          toast.error('حدث خطأ أثناء جلب الأحياء');
        }
      };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!selectedCategory || !selectedCity || !selectedDistrict) {
        toast.error('الرجاء اختيار التصنيف والمدينة والحي');
        return;
      }
      ...
      formData.append('district_id', selectedDistrict.toString());
